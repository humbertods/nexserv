...
// Versión: 3.8 - PANEL MIKAELA EN VIVO: HORAS FORMATEADAS (03/05/2026)
...
function handleFinalizarAtencion(data) {
  ...
  for (let i = 3; i < allData.length; i++) {
    ...
    // Buscar por nombre O código, case-insensitive en staff
    ...
    // Excluir tickets con fecha 1899 (atascados de pruebas)
    const fechaBruta = allData[i][1];
    const esFechaValida = fechaBruta instanceof Date && fechaBruta.getFullYear() > 2000;
    if (estado === 'en servicio' && matchStaff && (matchNombre || matchCodigo) && esFechaValida) {
      const row = i + 1;
