...
// Versión: 4.2 - VENTA PRODUCTOS: getMarcaProductos + registrarVentaProductos (04/05/2026)
...
// ============================================
// PRODUCTOS DE MARCA — lectura para venta al cobrar
// ============================================
function handleGetMarcaProductos() {
  try {
    // Intentar leer de la hoja Stock (SIRA) — columnas: nombre(B), stock(F), valor(J)
    // Si no existe, devolver lista vacía
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    let ws;
    try { ws = ss.getSheetByName('Stock'); } catch(e) {}
    if (!ws) try { ws = ss.getSheetByName('Catalogo'); } catch(e) {}
    if (!ws) return { success: true, productos: [] };

    const data = ws.getDataRange().getValues();
    const productos = [];
    for (let i = 1; i < data.length; i++) {
      const nombre = String(data[i][1] || '').trim();
      const stock = Number(data[i][5] || 0);
      const precio = Number(data[i][9] || data[i][3] || 0);
      if (nombre && precio > 0) {
...
function handleRegistrarVentaProductos(data) {
  try {
    ...
    // Guardar en CierresPagos — una fila por producto vendido
    const wsPagos = getSheet('CierresPagos');
    ...
    productos.forEach(p => {
      wsPagos.appendRow([
        now,                              // A: Fecha
        horaStr,                          // B: Hora
        data.clienteNombre || '',         // C: Cliente
        'Mikaela',                        // D: Vendido por
        '🛍 ' + p.nombre + (p.cantidad > 1 ? ' x' + p.cantidad : ''), // E: Producto
        Number(p.precio) * Number(p.cantidad || 1), // F: Total
        'Producto',                       // G: Tipo (no servicio)
        ''                                // H: idEspera referencia
      ]);
    });

    // También registrar en HistorialOwner para que aparezca en reportes
    try {
      ...
      wsHist.appendRow([
        ...
        data.total || 0,
        'Producto',   // metodoPago = Producto (para filtrar en reportes)
