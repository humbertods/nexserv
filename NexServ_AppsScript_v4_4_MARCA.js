...
// Versión: 4.4 - STOCK MARCA: hoja Marca corregida (B=nombre,C=stock,D=min,E=precio) (04/05/2026)
  ...
  for (let i = 3; i < allData.length; i++) {
    ...
    if (!matchId && !matchFallback) continue;
      ...
      if (data.nuevaArea && !data.esRetiro && !data.siguientePromo) {
        ...
        cerrarAtencion(data.idEspera, data.chicaNombre, data.clienteNombre, data.servicio, data.total, '', 'Por cobrar');
      }
      ...
      if (data.serviciosDetalle && data.serviciosDetalle.length > 0) {
        try {
          ws.getRange(row, 19).setValue(JSON.stringify(data.serviciosDetalle));
        } catch(e) {}
      }
...
// ============================================
// PRODUCTOS DE MARCA — conectado al Sheet de SIRA
// Sheet ID: 14GUyMUMbcQ_WQ9BY0iM5c4ApHN029w_uekis2udH2kA
// Hoja: Marca — columnas: A=id, B=Nombre, C=Stock, D=Min, E=Precio, F=Unidad
// Datos desde fila 2
// ============================================
const SIRA_SHEET_ID = '14GUyMUMbcQ_WQ9BY0iM5c4ApHN029w_uekis2udH2kA';

function getSiraMarcaSheet() {
  const ss = SpreadsheetApp.openById(SIRA_SHEET_ID);
  return ss.getSheetByName('Marca');
}

function handleGetMarcaProductos() {
  try {
    const ws = getSiraMarcaSheet();
    if (!ws) return { success: false, productos: [], error: 'Hoja Marca no encontrada en SIRA' };

    const lastRow = ws.getLastRow();
    if (lastRow < 2) return { success: true, productos: [] };

    // Fila 2 en adelante, columnas A-F
    const data = ws.getRange(2, 1, lastRow - 1, 6).getValues();
    const productos = [];

    for (let i = 0; i < data.length; i++) {
      const nombre  = String(data[i][1] || '').trim();  // col B
      const stock   = Number(data[i][2] || 0);           // col C
      const minimo  = Number(data[i][3] || 0);           // col D
      const precio  = Number(data[i][4] || 0);           // col E
      const rowNum  = i + 2;                             // fila real en el sheet

      if (!nombre) continue;

      productos.push({ nombre, stock, minimo, precio, rowNum });
    }
...
function handleRegistrarVentaProductos(data) {
  try {
    ...
    // 1. DESCONTAR STOCK EN SIRA (hoja Marca)
    try {
      const wsMarca = getSiraMarcaSheet();
      if (wsMarca) {
          ...
          productos.forEach(prod => {
            for (let i = 0; i < marcaData.length; i++) {
              ...
              if (nombreMarca.toLowerCase() === String(prod.nombre || '').toLowerCase()) {
                ...
                const minimo      = Number(marcaData[i][3] || 0); // col D
                const nuevoStock  = Math.max(0, stockActual - Number(prod.cantidad || 1));
                wsMarca.getRange(rowNum, 3).setValue(nuevoStock); // col C = Stock

                // Registrar en hoja Movimientos de SIRA
                try {
                  const ssSira = SpreadsheetApp.openById(SIRA_SHEET_ID);
                  const wsMov = ssSira.getSheetByName('Movimientos');
                  if (wsMov) {
                    wsMov.appendRow([
                      fechaStr,
                      nombreMarca,
                      ...
                      'Venta NexServ',
                      data.clienteNombre || 'Clienta',
                      'Unidad', 'Marca', horaStr
                    ]);
                  }
                } catch(eMov) {}
                break;
      ...
      Logger.log('Error descontando stock Marca: ' + eSira.toString());
    }
    ...
    try {
      ...
      wsHist.appendRow([
        ...
        data.clienteNombre || '',
        'Mikaela',
        productos.map(p => p.nombre + (p.cantidad > 1 ? ' x' + p.cantidad : '')).join(', '),
        data.total || 0,
        'Producto',
