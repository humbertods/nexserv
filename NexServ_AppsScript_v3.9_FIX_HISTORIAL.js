...
// Versión: 3.5 - FIX FECHA CIERRES PAGOS (30/04/2026)
...
function handleAddListaEspera(data) {
  ...
  // Columnas: A=ID | B=Fecha | C=Hora | D=Código | E=Nombre | F=Servicio | G=Área | H=Prioridad | I=Estado | J=TomadaPor | K=HoraToma | L=Obs | M=Total | N=PromoNombre | O=PrecioPromo | P=PrecioRegular
  ws.appendRow([
    id, fecha, hora, data.codigo, data.nombre, data.servicio,
    data.area, data.prioridad || 'Normal', estado, tomadaPor, '', data.observaciones || '',
    data.total || 0,                // M: Total del servicio normal
    data.promoNombre || '',          // N: Nombre de promo (si aplica)
    data.precioPromo || '',          // O: Precio de promo (si aplica)
    data.precioRegular || ''         // P: Precio regular (si aplica)
  ]);
    ...
    if (String(allData[i][0]).trim() === data.idEspera) {
      ...
      try {
        ...
        wsPagos.appendRow([
          ...
          ''                           // H: Servicios detalle (null para servicios simples)
        ]);
...
function handleGetHistorial(params) {
  ...
  const filtrados = periodo === 'hoy'
    ? historial.filter(h => h.fecha === hoy)
    : historial;

  // Agrupar por staff para el nuevo diseño del historial
  const porStaff = {};
  filtrados.forEach(h => {
...
function addHistorialOwner(atencion, data) {
  ...
  // Columnas ListaEspera: A=ID, B=Fecha, C=Hora, D=Codigo, E=Nombre, F=Servicio, G=Area, H=Prioridad, I=Estado, J=TomadaPor
  ws.appendRow([
    fecha,                    // A: Fecha
    hora,                     // B: Hora
    atencion[3],              // C: Codigo clienta (col D)
    atencion[4],              // D: Nombre clienta (col E)
    data.servicio || atencion[5] || '', // E: Servicio
    atencion[6] || data.area || '',     // F: Area (col G)
    atencion[9] || data.chicaNombre || '', // G: Staff nombre (col J = Tomada por)
    data.totalCobrado || data.montoChica || 0, // H: Total cobrado
    data.comision || 0,       // I: Comision
    data.metodoPago || 'Efectivo' // J: Metodo pago
  ]);
}
...
function handleAsignarServicioNormal(data) {
  // Actualizar ListaEspera con servicio normal asignado
  const ws = getSheet('ListaEspera');
  const rows = ws.getDataRange().getValues();
  
  for (let i = 3; i < rows.length; i++) {
    const row = rows[i];
    if (String(row[3]).trim() === String(data.codigo).trim()) {
      // Actualizar servicio, área y precio
      ws.getRange(i + 1, 6).setValue(data.servicio); // Servicio
      ws.getRange(i + 1, 7).setValue(data.area); // Área
      ws.getRange(i + 1, 13).setValue(data.precio); // Total/Precio
...
function handleAsignarPromo(data) {
  // Actualizar ListaEspera con promo asignada
  const ws = getSheet('ListaEspera');
  const rows = ws.getDataRange().getValues();
  ...
  for (let i = 3; i < rows.length; i++) {
    const row = rows[i];
    if (String(row[3]).trim() === String(data.codigo).trim()) {
      // Actualizar promo y precios
      ws.getRange(i + 1, 6).setValue(data.promoNombre + ' (PROMO)'); // Servicio
      ws.getRange(i + 1, 13).setValue(data.precioPromo); // Total
      ws.getRange(i + 1, 14).setValue(data.promoNombre); // Promo nombre
      ws.getRange(i + 1, 15).setValue(data.precioRegular); // Precio regular
...
function handleGetAutorizaciones() {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    // Solo devolver pendientes
    if (String(row[10]).toLowerCase() !== 'pendiente') continue;
