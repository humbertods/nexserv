...
function handleGetAutorizaciones() {
  ...
  // Retornar pendientes + aprobadas/rechazadas de las ultimas 2 horas
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    // Siempre incluir pendientes
    // Incluir aprobadas/rechazadas recientes para que el staff actualice su UI
    if (estado !== 'pendiente') {
      const fechaRow = row[1] instanceof Date ? row[1] : new Date(String(row[1] || ''));
      if (fechaRow < dosHorasAtras) continue;
    }
