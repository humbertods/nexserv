  ...
  try {
    ...
    rows.forEach((row, idx) => {
      ...
      const item = {
        ...
        areas,
        precioNormal:     Number(row[34] || 0),
        precioPromo:      Number(row[35] || 0),
    ...
    return { success: true, activos, porCobrar };
  } catch(e) { return { success: false, message: String(e) }; }
}
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      for (let a = 0; a < 4; a++) {
        ...
        if (data.precioNuevo) {
          ws.getRange(rowNum, TM_PRECIO_COL[a] + 1).setValue(Number(data.precioNuevo));
          // Recalcular totales
          const precios = TM_PRECIO_COL.map(c => Number(rows[i][c] || 0));
          precios[a] = Number(data.precioNuevo);
...
function handleConfirmarCobroMulti(data) {
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const metodoPago = data.metodoPago || 'Efectivo';
      // Precio según método: tarjeta → precioNormal, otros → precioPromo
      const esTargeta  = metodoPago === 'Tarjeta';
      const totalCobrado = esTargeta ? Number(rows[i][34] || 0) : Number(rows[i][35] || 0);
...
// ── Credenciales FCM V1 ──────────────────────────────────────
var FCM_PROJECT_ID    = 'nexserv-7e1bb';
var FCM_CLIENT_EMAIL  = 'firebase-adminsdk-fbsvc@nexserv-7e1bb.iam.gserviceaccount.com';
...
function getFCMAccessToken() {
  ...
  var signature = Utilities.base64EncodeWebSafe(
    Utilities.computeRsaSha256Signature(signInput, FCM_PRIVATE_KEY)
  ).replace(/=+$/, '');
