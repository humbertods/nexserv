...
function handleFinalizarAtencion(data) {
  ...
  for (let i = 3; i < allData.length; i++) {
    ...
    // Buscar por nombre exacto O por código de cliente (más robusto)
    ...
    if (estado === 'en servicio' && tomadaPor === data.chicaNombre && (matchNombre || matchCodigo)) {
      const row = i + 1;
      ...
      if (data.nuevaArea && !data.esRetiro && !data.siguientePromo) {
        ...
        // Actualizar obs con progreso
        const obsActual2 = String(allData[i][11] || '');
        const nuevaObs2 = (obsActual2 ? obsActual2 + ' | ' : '') + '✅ ' + (data.areaCompletada || user) + ' completado por ' + data.chicaNombre + ' · Sigue: ' + data.areasFaltantes;
